...
namespace WpfBookRentalShop01.ViewModels
{
    public partial class BookGenreViewModel : ObservableObject
    {
        ...
        {
            // 신규 추가/기존 데이터 수정
            //Debug.WriteLine(_selectedGenre.Names);
            //Debug.WriteLine(_selectedGenre.Division);
            ...
            try
            {
                ...
                using (MySqlConnection conn = new MySqlConnection(Common.CONSTR))
                {
                    ...
                    if (resultCnt > 0)
                    {
                        Common.LOGGER.Info("책 장르 데이터 저장 완료");
            ...
            LoadGridFromDb();   // 저장이 끝난 후 다시 DB 내용을 그리드에 그리기
        }
        ...
        public async void DelData()
        {
            ...
            try
            {
                ...
                using (MySqlConnection conn = new MySqlConnection(Common.CONSTR))
                {
                    ...
                    if (resultCnt > 0)
                    {
                        Common.LOGGER.Info($"책 장르 데이터 {SelectedGenre.Division} 삭제 완료");
